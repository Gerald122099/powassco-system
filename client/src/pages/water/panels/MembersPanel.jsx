...
export default function MembersPanel() {
  ...
  // Required-field rules differ by member type. Existing/migrated
  // members only need an account name — pnNo is auto-generated by
  // the server, mobile/birthdate/fullName get filled in later by
  // the officer through the UI. New members keep the full
  // validation gate.
  ...
  // For new members at least one meter NUMBER must be typed. For
  // existing members the meter number is auto-generated server-side,
  // so we only require that the meters[] array isn't empty.
  if (isExisting) {
              ...
              {!form.isExistingMember && (
                <Field label="PN No. (Account Number)" required>
                  <input
                    ...
                    disabled={!!editing}
                    placeholder="PN-001 (or leave blank to auto-generate)"
                  />
                </Field>
              )}
              {form.isExistingMember && !editing && (
            ...
            <div className="space-y-4">
              {form.meters.map((meter, index) => (
                <div key={index} className="p-4 border border-slate-200 rounded-xl bg-slate-50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <Field label="Meter Number" required={!form.isExistingMember}>
                      <input
                        className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5"
                        value={meter.meterNumber}
                        onChange={(e) => updateMeterInForm(index, 'meterNumber', e.target.value)}
                        placeholder={form.isExistingMember ? "Leave blank to auto-generate" : "e.g., MET-001"}
                      />
                    </Field>
