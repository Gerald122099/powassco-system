...
export default function PettyCashPanel() {
  ...
  const canWrite = ["admin", "cashier"].includes(user?.role); // others view-only
  ...
  return (
    <div className="space-y-4">
      <Card>
        ...
        {canWrite && balance <= 0 && (data?.count || 0) > 0 && (
          <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">
            Petty cash is empty — replenish the fund before issuing more vouchers.
          </div>
        )}
        {!canWrite && (
          <div className="mt-3 text-xs text-slate-400">View-only — petty cash is managed by the cashier.</div>
        )}
      </Card>
